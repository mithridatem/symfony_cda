...
class ApiArticlesController extends AbstractController{
    ...
    #[Route('/api/articles/get/id/{id}', name:'app_api_articles_id')]
    public function getArticlesId(ArticleRepository $repo, ApiRegister $apiRegsiter, Request $request, $id):Response{
        //vérifier le token 
        //récupérer la clé
        $secretKey = $this->getParameter('token');
        //récupérer le token 
        ...
        if($verif===true){
            ...
            if($data){
                ...
                return $this->json(['Error'=>'L\'article n\'existe pas en BDD'], 206, ['Content-Type'=>'application/json',
                'Access-Control-Allow-Origin'=> '*']);
            }
        }
        //sinon on envoyer un json d'erreur (pas authorisé)
